...
def drive_joint(current_th1, current_th2, current_th3, current_speed_1, current_speed_2, current_speed_3, delta_t):
    new_th1 = current_th1 + (current_speed_1/delta_t)
    new_th2 = current_th2 + (current_speed_2/delta_t)
    new_th3 = current_th3 + (current_speed_3/delta_t)
    return new_th1, new_th2, new_th3
...
while t < 0.003:
    ...
    print("Giro numero: ", len(time)," posizione stimata e alpha vale: -->",x_current_provv, y_current_provv, alpha_c, " con angoli th1, th2, th3 -->", current_th1, current_th2, current_th3)
    target_th1, target_th2, target_th3 = cinematica_inversa(Xt, Yt, alpha_t)
    ...
    current_speed_1 = speed_controller.evaluate(target_speed_1, current_speed_1, delta_t)
    print("Giro numero: ", len(time), " target_speed-1 = ", target_speed_1, ", current_speed_1 aggiornato vale: ", current_speed_1)
    target_speed_2 = position_controller.evaluate(target_th2, current_th2, 0, delta_t)
    current_speed_2 = speed_controller.evaluate(target_speed_2, current_speed_2, delta_t)
    print("Giro numero: ", len(time), " target_speed-2 = ", target_speed_2, ", current_speed_2 aggiornato vale: ", current_speed_2)
    target_speed_3 = position_controller.evaluate(target_th3, current_th3, 0, delta_t)
    current_speed_3 = speed_controller.evaluate(target_speed_3, current_speed_3, delta_t)
    print("Giro numero: ", len(time), " target_speed-3 = ", target_speed_3, ", current_speed_3 aggiornato vale: ", current_speed_3)
    print("Giro numero: ", len(time), " prima l'aggiornamento i vari current valgono: ", current_th1, current_th2, current_th3)
    current_th1, current_th2, current_th3 = drive_joint(current_th1, current_th2,current_th3,current_speed_1, current_speed_2, current_speed_3, delta_t)
    print("Giro numero: ", len(time), " dopo l'aggiornamento i vari current valgono: ", current_th1, current_th2, current_th3)
...
plt.plot(x_current, y_current, label='output')
plt.plot(x_target, y_target, label='riferimento')
plt.xlabel('time')
plt.legend()
